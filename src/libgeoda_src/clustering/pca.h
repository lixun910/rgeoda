...
private:
    ...
    std::string _method;     // svd, cor, cov
                             // Numbers of eliminated columns
    std::vector<unsigned int> _eliminated_columns;
    std::vector<float>  _sd;           // Standard deviation of each component
    std::vector<float> _prop_of_var;   // Proportion of variance
    std::vector<float> _cum_prop;      // Cumulative proportion
    std::vector<float> _scores;        // Rotated values
    unsigned int  _kaiser;             // Number of PC according Kaiser criterion
    unsigned int _thresh95;            // Number of PC according 95% variance threshold

public:
    //! Class constructor
    Pca(const std::vector<std::vector<double> >& x,  const unsigned int &nrows, const unsigned int &ncols);
    //! Class destructor
    ~Pca(void);
