...
`rgeoda` is an R library for spatial data analysis. It is an R wrapper of the libgeoda C++ library, which is built based on the `GeoDa` software. The version used in this tutorial is version 0.0.4.


## 1. Install `rgeoda`

Like `GeoDa` desktop software, `rgeoda` is avaiable to different platforms including: Mac, Linux and Windows. 
...
The data formats that `rgeoda` can read directly includes: ESRI Shapefile (Geojson later).
...
Methods for *bivariate* and *multivariate local* spatial autocorrelation statistics, as well as *global* spatial autocorrelation satatistics, will be included in the next release of `rgeoda`.

In this tutorial, we will only introduce how to call these methods using `rgeoda`. Please read [here](http://geodacenter.github.io/workbook/6a_local_auto/lab6a.html) for more information about the local spatial autocorrelation statisticis. 
...
The original AZP heuristic is a local optimization procedure that cycles through a series of possible swaps between spatial units at the boundary of a set of regions. The process starts with an initial feasible solution, i.e., a grouping of n spatial units into p contiguous regions. This initial solution can be constructed in a number of different ways. It is critical that the initial solution satisfies the contiguity constraints. For example, this can be accomplished by growing a set of contiguous regions from p randomly selected seed units by adding neighboring locations until the contiguity constraint can no longer be met.
...
```{r}
maxp_sa_clusters <- maxp_sa(queen_w, data, bound_vals, min_bound, cool_rate=0.85, sa_maxit=1)
...
```{r}
maxp_clusters <- maxp_greedy(queen_w, data, bound_vals, min_bound, iterations=200)
