...
NOTE: please install it from github source for testing.  
...
`rgeoda` uses `GeoDa`’s C++ code, in which multi-threading is used to accelerate the computation of LISA. We can use the argument `ncpu` to specify how many threads to run the computation:
...
Other standardization functions include: `demean()` and `mad()`, which are same in the `GeoDa` program.
```{r}
